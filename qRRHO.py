...
#3/21/2017 Add surface area for ZSM-5 Jack
# The format to execute this file is extract.py #1 #2 #3. #1 is the filename, #2 and #3 are desired T & P.
import sys
import os
import ast
import shutil as sh
...
parser.add_argument('-z', '--zeolite',default='BEA',
                    type=str, help='Type of zeolite')
...
if args.zeolite == 'BEA':
    L1 = 800 * 10**-12  
    L2 = 800 * 10**-12  #surface area of BEA zeolite is 800x800 pm
if args.zeolite == 'ZSM':
    L1 = 600 * 10**-12
    L2 = 200 * 10**-12  #surface area of ZSM zeolite is 600x200 pm
A = L1*L2 #m^2 surface area of BEA
...
q_t_1d_1 = math.sqrt(2*pi*m_ads1*k_B*T/h**2)*L1
q_t_1d_2 = math.sqrt(2*pi*m_ads2*k_B*T/h**2)*L1
if args.mass1 == 0 :
    q_t_1d_1 = 1
if args.mass2 == 0 :
    q_t_1d_2 = 1
...
name=str.split(file,'.')[0]+'_'+str(int(T))+'_all.txt'
f=open(name,'w')
...
if args.mass1:
    f.write('If we consider vibrational degrees of freedom and 2D translational motion and use q_RRHO,the electronic and zero-point, electronic and thermal, electronic and enthalpy, electronic, free energy and S are:\n')
...
f.write('Use harmonic oscillator and keep all the degrees of freedom. The thermal, enthalpy and entropy corrections are:\n')
...
f.write('If we only consider vibrational degrees of freedom and use q_RRHO, The thermal, enthalpy and entropy corrections are:\n')
